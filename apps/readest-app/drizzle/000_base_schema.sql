-- Migration 000: base schema for a fresh database.
--
-- Verbatim copy of docker/volumes/db/init/schema.sql (the DDL upstream applies
-- to a new self-hosted install), minus its two leading
-- `ALTER FUNCTION auth.* OWNER TO supabase_auth_admin` statements: on Supabase
-- Cloud those functions already have that owner and the `auth` schema is not
-- ours to alter.
--
-- Upstream keeps init/schema.sql current by folding column additions into it
-- alongside the matching NNN migration, so this copy never needs refreshing —
-- anything added after it was taken arrives through the numbered migrations,
-- all of which use IF NOT EXISTS for columns and tables.
