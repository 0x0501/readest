...
const fileFromZip = async (blob: Blob, name: string) =>
  new File([new Uint8Array(await blob.arrayBuffer())], name, { type: 'application/zip' });
...
const createDictionary = async (): Promise<File> => {
  ...
  return fileFromZip(await writer.close(), 'reader-japanese.zip');
};
...
const createDictionaryWithTerms = async (
  ...
  return fileFromZip(await writer.close(), 'lookup-limits.zip');
};
...
describe('Yomitan importer and lookup', () => {
  ...
  test.each([
    ...
    const source = await fileFromZip(await writer.close(), 'invalid-banks.zip');
    db = await NodeDatabaseService.open(':memory:');
  ...
  test('does not claim an arbitrary ZIP', async () => {
    ...
    const source = await fileFromZip(await writer.close(), 'notes.zip');
    db = await NodeDatabaseService.open(':memory:');
