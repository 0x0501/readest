...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  return withDb(async (db) => {
    ...
    // flood R2 once the user has too many pending items. Scoped by user_id,
    // which is the whole of the authorization here (ADR-005).
    ...
    try {
      await db.update(schema.sendInbox).set({ payloadKey }).where(eq(schema.sendInbox.id, row.id));
    } catch (error) {
      await db.delete(schema.sendInbox).where(eq(schema.sendInbox.id, row.id));
      try {
