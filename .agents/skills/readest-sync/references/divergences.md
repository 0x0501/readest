...
| `apps/readest-app/src/libs/auth/{client,mail,verify}.ts` | Better Auth browser client, the Cloudflare mail sender, JWKS verification |
| `apps/readest-app/src/hooks/useTurnstile.tsx` | Captcha widget, absent when unconfigured |
| `apps/readest-app/src/app/auth/{forgot-password,reset-password}/` | Password reset, which upstream has no equivalent of (ADR-016) |
| `apps/readest-app/src/app/user/components/PasskeySection.tsx` | Passkey enrolment and removal (ADR-018) |
| `apps/readest-app/drizzle/local_004_passkey.sql` | `passkey` table. Generated by Better Auth's migration compiler, never hand-written |
| `apps/readest-app/src/__tests__/database/auth-gate.pg.test.ts` | Allow-list, captcha and reset driven over real HTTP |
