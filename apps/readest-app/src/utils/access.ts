...
// `AuthProvider` writes both of these on every session change and clears them on
// sign-out, so localStorage is the whole of the client's view of who is signed
// in. The bearer token it holds is Better Auth's JWT, minted from the session
// cookie by `/api/auth/token`.
